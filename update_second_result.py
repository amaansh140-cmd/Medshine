...
start_idx = content.find("<!-- Result 2 -->")
end_idx = content.find("<!-- Result 3 -->")
...
if start_idx != -1 and end_idx != -1:
  ...
  <div class="flex flex-col gap-6 group reveal-anim">
    ...
    <div class="relative w-full aspect-[4/3] rounded-[24px] overflow-hidden group/slider">
      <!-- After Image -->
      <img src="/src/assets/after_2.jpg" alt="After Treatment" class="absolute inset-0 w-full h-full object-cover" />
      
      <!-- Before Image Container (clipped) -->
      <img src="/src/assets/before_2.jpg" alt="Before Treatment" class="absolute inset-0 w-full h-full object-cover [clip-path:polygon(0_0,var(--split,50%)_0,var(--split,50%)_100%,0_100%)]" id="ba-image-2" />

      <!-- Range Input for dragging -->
      <input type="range" min="0" max="100" value="50" class="absolute inset-0 w-full h-full opacity-0 cursor-ew-resize z-20" oninput="document.getElementById('ba-image-2').style.setProperty('--split', this.value + '%'); document.getElementById('ba-handle-2').style.left = this.value + '%';" />

      <!-- Visual Handle -->
      <div id="ba-handle-2" class="absolute inset-y-0 left-1/2 w-1 bg-white/80 pointer-events-none z-10 -ml-[2px]">
    ...
    <div class="flex flex-col gap-2">
      <h3 class="font-serif text-2xl font-semibold text-ink leading-tight">Advanced Hair Restoration</h3>
      <p class="text-inkmute font-light leading-relaxed">Visible increase in hair density and thickness at the crown area following a customized clinical protocol of regenerative therapy and nutritional support.</p>
    </div>
  </div>
  ...
  """
    ...
    with open("results.html", "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Added before/after slider to Result 2 in results.html!")
else:
    print("Could not find Result 2 markers.")
